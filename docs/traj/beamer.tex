...
\begin{itemize}
\item $\mathcal{H}$, $\Lambda$, $\Phi$: altitudes ($[0~\text{km},20~\text{km}]$), longitudes ($[0, 2\pi]$), latitudes ($[-\pi/2,\pi/2]$). Physically intuitive to keep as continuous variables---curse of dimensionality as we discretize.
\item $\mathcal{T}$: time, defined in discrete steps of 10~min. Allows us to use simple Euler integration spatially to trace out wind field (altitude handled differently).
\end{itemize}
...
\end{itemize}
\item Optimize $V$ via gradient descent, $\theta^{k+1} = \theta^k + \nabla_\theta V(\theta)$.
...
\begin{itemize}
\item Suppose we want to optimize final longitude after $N$ steps (say $N=720$, 5 days).
