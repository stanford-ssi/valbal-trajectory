...
def fetchWindData(start, end, download_dir, db='gfs_anl_1deg'):
    """ Fetch data from gfs database for times inbetween start and end
    """
    if db == 'gfs_anl_1deg':
        remote = "https://nomads.ncdc.noaa.gov/data/gfsanl/"
        local = download_dir + "/raw/gfs_anl_1deg0/"
        fstartname = "gfsanl_3_"
    if db == 'gfs_anl_0deg5':
        remote = "https://nomads.ncdc.noaa.gov/data/gfsanl/"
        local = download_dir + "/raw/gfs_anl_0deg5/"
...
def procWindData(start, end, download_dir, db='gfs_anl_1deg', overwrite=False):
    dstpath = download_dir + "/proc/" + db + "/"
    srcpath = download_dir + "/raw/" + db + "/"
    ret = fetchWindData(start, end, download_dir, db)
    times = ret[1]
    files = ret[0]
...
def makeWindArray(start,end,download_dir,overwrite=False,altitude_range=[10000,20000],name_modifier=''):
    ...
    local = download_dir + "/GFS-anl-0deg5/"
    filepath = download_dir + "/GFS_anl_0deg5_objs/" + start + "_to_" +end + name_modifier+ ".pickle"
    filepath2 = download_dir + "/GFS_anl_0deg5_objs/" + start + "_to_" +end + name_modifier+".npy"
...
def p2a(x):
    """ Pressure to altitude hectopascals to meters
    """
    return (1-(x/1013.25)**0.190284)*145366.45*0.3048


if __name__ == '__main__':
    download_dir = sys.argv[1] if len(sys.argv) >= 2 else '../ignored'
    location_source = sys.argv[2] if len(sys.argv) >= 3 else '../ignored/flights/ssi71_location.h5'
    print('Reading %s and downloading to %s' % (location_source, download_dir))

    df = pd.read_hdf(location_source)
    print(df.long_gps.values[1000])
    print(df.lat_gps.values[1000])
    print(df.index[0])
    procWindData(df.index[0],df.index[-1] + timedelta(2),download_dir,db="gfs_anl_0deg5",overwrite=False)
